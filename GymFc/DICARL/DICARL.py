...
def traj_segment_generator(pro_pi, adv_pi, env, alpha, horizon, max_steps_episode, stochastic, clip_action=False,):
    t = 0
    print('M_d', alpha)
...
def build_l(s, a, s_dim, a_dim,reuse=None, custom_getter=None):
    ...
    with tf.variable_scope('Lyapunov', reuse=reuse, custom_getter=custom_getter):
        n_l1 = 32  # 30 # 正儿八经用的时候L网络又和Q网络的参数不一样了，bitch
        ...
        return tf.layers.dense(net_0, 1, trainable=trainable)  # L(s,a)
...
def learn(env, policy_func_pro, policy_func_adv, *,
    ...
    labda = tf.placeholder(tf.float32, None, 'LR_Labda')
    ...
    with tf.control_dependencies(target_update):
        ...
        Lyapunov_train = [ltrain] #, lambda_train]
    ...
    print('info:', 'alpha', lr_alpha, 'Finite_horizon',finite_horizon, 'adv_mag', env.adv_action_space.high, 'timesteps', max_timesteps)
    ...
    # with tf.Session() as sess:  # 用这段代码生成图
    #     # 生成一个具有写权限的日志文件操作对象，将当前命名空间的计算图写进日
    #     writer = tf.summary.FileWriter('/home/jianghan/My_Project/Robust_RL/RARL1/log/Graph/fuck-l2-v5', sess.graph)
    ...
    while True:
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                ...
                Lyapunov_opt(batch["rew"].reshape(-1,1), l_value.reshape(-1,1), batch["new"].reshape(-1,1), batch["ob"], batch["ob_"], batch["ac"],
