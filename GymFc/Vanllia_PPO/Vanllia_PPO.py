...
def traj_segment_generator(pi, env, horizon, stochastic, action_clip=True, flight_log=None):
    t = 0
...
def learn(env, policy_fn, *,
    ...
    # ----------------------------------------
    seg_gen = traj_segment_generator(pi, env, timesteps_per_actorbatch, action_clip=action_clip,
    ...
    while True:
        ...
        if MPI.COMM_WORLD.Get_rank()==0:
            logger.dump_tabular()
            # print("EpLenMean", np.mean(lenbuffer), "EpRewMean", np.mean(rewbuffer), "EpThisIter", len(lens), "TimestepsSoFar", timesteps_so_far)
