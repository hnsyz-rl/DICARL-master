...
if __name__ == "__main__":
    ...
    eval_dir = './log/gymfc_nf-step-v1/RARL-1e7-action-clip-0408-adv_0_01-attack_n-3e4/evaluations_noise'
    ckpts = glob.glob(eval_dir + "/*/")
    ...
    for ckpt in ckpts:
        ...
        num_ckpts += 1
        shaw_ckpt = ckpt.replace(eval_dir,'')
        print('Num%d'%num_ckpts, 'Name:', shaw_ckpt.replace('/',''), ' MAE: %.3f'%metrics[0], 'Ave. Return: %.3f'%metrics[3])
    ...
    ax[-1].set_xlabel("Checkpoint_RARL")
    plt.show()
