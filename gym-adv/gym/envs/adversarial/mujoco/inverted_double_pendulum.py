...
class InvertedDoublePendulumEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self):
        ...
        mujoco_env.MujocoEnv.__init__(self, 'inverted_double_pendulum_zp.xml', 5)
    ...
    def _step(self, action):
        ...
        ob = self._get_obs()
        # site_name = b'tip' # zp
        # site_index = self.model.site_names.index(site_name)  # zp
        # x, _, y = self.model.data.site_xpos[site_index]  # zp
        x, _, y = self.model.data.site_xpos[0]  # zp
        theta1 = self.model.data.qpos[1]
        theta2 = self.model.data.qpos[2]
        # dist_penalty = 2 * (1.2 - y)**2 + (theta1**2 + theta2**2) *2
        dist_penalty = 0.01 * x ** 2 + (y - 2) ** 2
        # dist_penalty = (abs(np.sin(theta1)) + abs(np.sin(theta2)))  *10
        v1, v2 = self.model.data.qvel[1:3]
        vel_penalty = 1e-3 * v1**2 + 5e-3 * v2**2
        r = (self.alive_bonus - dist_penalty - vel_penalty)[0]
