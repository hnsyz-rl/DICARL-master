...
class MlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, tau, gaussian_fixed_var=True):
        ...
        self.ob = U.get_placeholder(name="ob_adv", dtype=tf.float32, shape=[sequence_length] + list(ob_space.shape))
        self.ob_ = U.get_placeholder(name="adv_ob_", dtype=tf.float32, shape=[sequence_length] + list(ob_space.shape))
        ...
        with tf.variable_scope('adv_vf'):
            ...
            for i in range(self.num_hid_layers):
                last_out = tf.nn.tanh(tf.layers.dense(last_out, self.hid_size, name="adv_vffc%i"%(i+1), kernel_initializer=U.normc_initializer(1.0)))
            self.vpred = tf.layers.dense(last_out, 1, name="adv_vffinal", kernel_initializer=U.normc_initializer(1.0))[:,0]
    ...
    def build_action(self, s, name='adv_actor', reuse=None, custom_getter=None):
        if reuse is None:
            trainable = True
            last_out = self.obz
        else:
        ...
        with tf.variable_scope(name, reuse=reuse, custom_getter=custom_getter):
            for i in range(self.num_hid_layers):
                last_out = tf.nn.tanh(tf.layers.dense(last_out, self.hid_size, name="adv_polfc%i" % (i + 1),
                                                      kernel_initializer=U.normc_initializer(1.0),
                                                      trainable=trainable))
            if self.gaussian_fixed_var and isinstance(self.ac_space, gym.spaces.Box):
                mean = tf.layers.dense(last_out, self.pdtype.param_shape()[0] // 2, name="adv_polfinal",
                                       kernel_initializer=U.normc_initializer(0.01),
                                       trainable=trainable)
                logstd = tf.get_variable(name="adv_logstd", shape=[1, self.pdtype.param_shape()[0] // 2],
                                         initializer=tf.zeros_initializer(),
                                         trainable=trainable)
                pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)
            else:
                pdparam = tf.layers.dense(last_out, self.pdtype.param_shape()[0], name="adv_polfinal",
                                          kernel_initializer=U.normc_initializer(0.01),
                                          trainable=trainable)
        return pdparam
