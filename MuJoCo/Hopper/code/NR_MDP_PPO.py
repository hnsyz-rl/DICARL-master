...
def traj_segment_generator(pro_pi, adv_pi, env, alpha, horizon, max_steps_episode, stochastic, clip_action=True):
    ...
    while True:
        ...
        if clip_action:
            ...
            adv = a_lowerbound + (np.tanh(adv_ac) + 1.) * (a_upperbound - a_lowerbound) / 2
        else:
            pro = pro_ac
            adv = adv_ac
...
def learn(env, policy_func, policy_fn_adv, *,
        ...
        gamma, lam, alpha,# advantage estimation
    ...
    adv_ac_space = env.action_space  # 这里是动作干扰，所以干扰的动作空间同主智能体
    ...
    while True:
        ...
        logger.log("*********NR_MDP*Iteration %i **Percentage %.2f **********" % (iters_so_far, Percentage))
